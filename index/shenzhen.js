...
var Shenzhen=React.createClass({
			...
			<div>
				{
					this.state.reshead.map(function(item,index){
						return (<Link to={"/shenzhen-detail/"+item.id} key={index}><Childswiper name={item} /></Link>)
					})
				}
				<p style={{'fontSize':'5px','color':'#999','margin':'2vw'}}>深圳大事件</p>
				{
					this.state.reslist.map(function(item,index){
						return (<Link to={"/shenzhen-detail/"+item.id} key={index}><Childlist name={item} /></Link>)
					})
				}
				{
					this.state.resfoot.map(function(item,index){
						return (<Link to={"/shenzhen-detail/"+item.id} key={index}><Childfoot name={item} /></Link>)
					})
				}
